...
if __name__ == '__main__':
    ...
    video_file = 'device_power/experiments_miso_caiso.mp4'
    ...
    full['MISO (g CO2 e)'] = (full['Energy (J)']/3600000)*(full['MISO g CO2 e/kWh'])
    full['CAISO (g CO2 e)'] = (full['Energy (J)']/3600000)*(full['CAISO g CO2 e/kWh'])
    
    full.to_csv('experiment_results/results.csv', index=False)

    # plot eneregy versus carbon emissions and connect CAISO to MISO
    ax = full.plot.scatter('Energy (J)','CAISO (g CO2 e)',c="b",label="CAISO")
    full.plot.scatter('Energy (J)','MISO (g CO2 e)',c="r",label="MISO", ax=ax)
    for n,row in full.iterrows():
        ax.plot([row['Energy (J)']]*2,row[['CAISO (g CO2 e)', 'MISO (g CO2 e)']], color="gray", lw=1, zorder=0, linestyle='dotted')
    
    ax.set_ylabel('Embodied carbon (g CO2 e)')
